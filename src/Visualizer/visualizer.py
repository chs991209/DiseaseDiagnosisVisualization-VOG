...
class VOGMatplotlibVisualizer:
    def plot(self, data: VOGData) -> None:
        ...
        # View 1: Raw Signal
        axes[0].plot(df[t_col], df[data.target_col], label=f'Target', color='red', linestyle='--')
        ...
        axes[1].plot(df[t_col], df['Error_L'], label='Error (L)', color='purple', alpha=0.8)
        axes[1].plot(df[t_col], df['Error_R'], label='Error (R)', color='orange', alpha=0.8)
        axes[1].set_title('2. Derived Feature: Eye Tracking Error')
        ...
        if data.noise_col_l and data.noise_col_r:
            ...
            axes[2].set_title('3. Outlier/Noise Monitoring (Cross-Axis)')
        else:
